...
var stage = null;
let startX = maskElement.width;
let startY = maskElement.height;
...
window.addEventListener('load', function () {
  ...
  particleSystem.importFromJson(
    ...
    "startY": 218,
    "startYVariance": 1024,
    "initialDirection": 4.5,
    "initialDirectionVariance": "360",
    "initialSpeed": 8.5,
    "initialSpeedVariance": 0.1,
    "friction": "0.063",
    "accelerationSpeed": 2.0045,
...
function handleTick() {
  ...
  if (startX >= 0 && startX <= maskElement.width){
    particleSystem.startX = startX;
  }
  if (startY >= 0 && startY <= maskElement.height){
...
let canvasCtx = guideCanvas.getContext('2d');
  ...
  drawConnectors(canvasCtx, results.poseLandmarks, POSE_CONNECTIONS, {
    ...
    */
   startX = (results.poseLandmarks[24].x + results.poseLandmarks[23].x) /2.0 * maskElement.width
   startY = (results.poseLandmarks[24].y + results.poseLandmarks[23].y) /2.0 * maskElement.height
  
  //canvasに3D(rendereの中身)を描画
